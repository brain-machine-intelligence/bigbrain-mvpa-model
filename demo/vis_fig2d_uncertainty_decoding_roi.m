...
% ROI = {'F3TL', 'F3TR', 'DLPFCL', 'DLPFCR', 'OFC', 'ACCL', 'ACCR', 'SMAL', 'SMAR', 'V1L', 'V1R', 'HIPPOL', 'HIPPOR', 'VentStrL', 'VentStrR'};
% ROI = {'F3TL', 'F3TR', 'ACCL', 'ACCR', 'SMAL', 'SMAR', 'V1L', 'V1R', 'HIPPOL', 'HIPPOR', 'VentStrL', 'VentStrR'};
% ROI = {'F3TL', 'F3TR'};
% ROI = {'DLPFCL', 'DLPFCR'};
% ROI = {'ACCL', 'ACCR'};
% ROI = {'SMAL', 'SMAR'};
...
LABEL = {'UncCond G', 'UncCond H'}; N_allclass = 2 * ones(1, length(LABEL)); 
            ...
            for labi = 1:length(LABEL)
                ...
                for evi = 1:length(EVENT)
                    ...
                    try
                        ...
                        if isfield(data_load.save_info, 'accs_r') && strcmp(measure, 'accs_r')
                            mean_msr_box(id, labi, roii, evi) = mean(data_load.save_info.accs_r(:)); % (accs_r: Repeat, rRepeat)
                        end
...
% --- blkCond clsf types ---
switch Exp
    case {'Lee2014', 'Lee2014pp2', 'Heo2018'}
        if newrun
            interaction_clsf = 5;
            goalcond_clsf = 3;
            unccond_clsf = 6;
...
elseif strcmp(measure, 'accs') || strcmp(measure, 'acc_mean')
    ...
    if contains(Exp, {'Lee2014', 'Heo2021'})
        y_lower = chance_lvl - 0.01;
        y_upper = chance_lvl + 0.055;
    elseif contains(Exp, 'Kim2019')
        y_lower = chance_lvl - 0.04;
...
switch vis_mode
    ...
    case 'regional_info'
        ...
        % Comparison boxplot
        % Binary comparison between the two labels
        if length(vLABELi) == 2
            msrmat1 = squeeze(msrmat(:, 1, :));
            msrmat2 = squeeze(msrmat(:, 2, :));
            msr_diff_mat = msrmat1 - msrmat2; % [N_sbj x N_roi]

            % Significance check & marking
            [h, p] = ttest(msrmat1, msrmat2);
            colors_temp = 0.4 * ones(length(vROIi), 3); 
            if any(h)
                colors_temp(logical(h), :) = repmat([0.6 0.4 0.3], sum(logical(h)), 1); 
            end
...
%                     'Alpha', sgnf_lvl);
                ...
                fprintf([ROI{vROIi(vroii)} ': t(%d)=%.3f\t p=%.3e (' sig_text{vroii} ')\n'], stats.df, stats.tstat, p)
            end
        ...
        B = bar(msrmat_sbjmean', 1, 'EdgeColor', 'none'); axis square; box off;
        % BIG BRAIN 3rd seminar & COSYNE 2023 poster fig 2a - Goal, S2, S3 SD
%         B(1).FaceColor = [77 161 169]/255;
%         B(2).FaceColor = [255 166 48]/255;
%         B(3).FaceColor = [215 232 186]/255;
%         % RLDM 2022 - UC G, UC H SD
        B(1).FaceColor = [0.85 0.35 0.45];
        B(2).FaceColor = [0.25 0.3 0.9];

        % Coloring for trial delay/advance
        if length(vLABELi) == 6

            B(1).FaceColor = [0.85 0.35 0.45]; % G
            B(2).FaceColor = .8 * [0.85 0.35 0.45]; % G delay 1
            B(3).FaceColor = .6 * [0.85 0.35 0.45]; % G delay 2

            B(4).FaceColor = [0.25 0.3 0.9]; % H
            B(5).FaceColor = .8 * [0.25 0.3 0.9]; % H delay 1
            B(6).FaceColor = .6 * [0.25 0.3 0.9]; % H delay 1

        elseif length(vLABELi) == 10

            B(1).FaceColor = 1.2 * [0.85 0.35 0.45] - mod(1.2 * [0.85 0.35 0.45], 1); % G advance 2
            B(2).FaceColor = [0.85 0.35 0.45]; % G advance 1
            B(3).FaceColor = .8 * [0.85 0.35 0.45]; % G
            B(4).FaceColor = .6 * [0.85 0.35 0.45]; % G delay 1
            B(5).FaceColor = .4 * [0.85 0.35 0.45]; % G delay 2

            B(6).FaceColor = 1.2 * [0.25 0.3 0.9] - mod(1.2 * [0.25 0.3 0.9], 1); % H advance 2
            B(7).FaceColor = [0.25 0.3 0.9]; % H advance 1
            B(8).FaceColor = .8 * [0.25 0.3 0.9]; % H
            B(9).FaceColor = .6 * [0.25 0.3 0.9]; % H delay 1
            B(10).FaceColor = .4 * [0.25 0.3 0.9]; % H delay 2

            % B(1).FaceColor = 1.4 * [0.85 0.35 0.45] - mod(1.4 * [0.85 0.35 0.45], 1); % G advance 2
            % B(2).FaceColor = 1.2 * [0.85 0.35 0.45] - mod(1.2 * [0.85 0.35 0.45], 1); % G advance 1
            % B(3).FaceColor = [0.85 0.35 0.45]; % G
            % B(4).FaceColor = .8 * [0.85 0.35 0.45]; % G delay 1
            % B(5).FaceColor = .6 * [0.85 0.35 0.45]; % G delay 2
            % 
            % B(6).FaceColor = 1.4 * [0.25 0.3 0.9] - mod(1.4 * [0.25 0.3 0.9], 1); % H advance 2
            % B(7).FaceColor = 1.2 * [0.25 0.3 0.9] - mod(1.2 * [0.25 0.3 0.9], 1); % H advance 1
            % B(8).FaceColor = [0.25 0.3 0.9]; % H
            % B(9).FaceColor = .8 * [0.25 0.3 0.9]; % H delay 1
            % B(10).FaceColor = .6 * [0.25 0.3 0.9]; % H delay 2
...
%         xtickangle(45)
        ...
        if length(vLABELi) == 2
            diff_erbr = .13; % two labels, six ROIs (original main setting)
            errLineWidth = 3;
        elseif length(vLABELi) == 6
            diff_erbr = .33; % six labels, eight ROIs
            errLineWidth = 3;
        elseif length(vLABELi) == 10
            diff_erbr = .35; % ten labels, eight ROIs
            errLineWidth = 2;
        end
        % diff_erbr = .175; % four bars
%         diff_erbr = .225;
        % diff_erbr = .25; % three bars
%         diff_erbr = 0;
...
%         end
        ...
        E = errorbar(xtick_erbr, reshape(msrmat_sbjmean,1,[]), ... 
            reshape(msrmat_sbjste,1,[]), 'LineStyle', 'none', 'LineWidth', errLineWidth);
        % errorbar LineWidth: original - 1.2, bold (COSYNE 2023 poster fig 2a) - 2
        E.Color = colors(21, :); 
