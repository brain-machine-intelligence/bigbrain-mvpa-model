...
% label_name = 'Goal(6,7,8)xUC LOROV goalOnly'; temp_color = [77 161 169]/255; % repeat 100 SD
% label_name = 'Goal(6,7,8)xUC LOROV uncOnly'; temp_color = [226 16 103]/255; % repeat 100 SD
% label_name = 'Goal(6,7,8)xUC LOROV factorization'; temp_color = [155 80 148]/255; % repeat 100 SD
% label_name = 'Goal(6,7,8)xUC LOROV nonlinearMix'; temp_color = [94 35 157]/255; % repeat 100 SD  
...
% corr_behav_dir = 'corr_behav_sub1920'; neural_measure = 'Separability dimension';
% corr_behav_dir = 'corr_behav_sub51000'; neural_measure = 'Separability dimension';
% corr_behav_dir = 'corr_behav_sub101000'; neural_measure = 'Separability dimension';
% corr_behav_dir = 'corr_behav_sub151000'; neural_measure = 'Separability dimension';
% corr_behav_dir = 'corr_behav_sub201000'; neural_measure = 'Separability dimension';
...
%         pval_cell = cell(length(ROI), length(BEHAV));
        ...
        for roii = 1:length(ROI)
            for bhvi = 1:length(BEHAV)
                for evi = 1:length(EVENT)
                    ...
                    exp_name = 'Lee2014';
                    file_name = ['data\shattering_result\' corr_behav_dir '\' num2str(EVENT(evi)) '\' ...
    ...
    case 'corr_behav_sub1920'

        coef_mat = nan(20, length(ROI), length(BEHAV), length(EVENT));
        pval_mat = nan(20, length(ROI), length(BEHAV), length(EVENT));
        ...
        for roii = 1:length(ROI)
            for bhvi = 1:length(BEHAV)
                for evi = 1:length(EVENT)
                    ...
                    coef_mat(:, roii, bhvi, evi) = Rs;
                    pval_mat(:, roii, bhvi, evi) = ps;
    ...
    case {'corr_behav_sub51000', 'corr_behav_sub101000', 'corr_behav_sub151000', 'corr_behav_sub201000'}

        coef_mat = nan(1000, length(ROI), length(BEHAV), length(EVENT));
        pval_mat = nan(1000, length(ROI), length(BEHAV), length(EVENT));
...
%             figure('Name', ROI{roii});
            ...
            % figure('Position', [0 1080/2-80 1920/12 1080/2]) % [left bottom width height]
            % figure('Position', [0 1080/2-80 1920/8 1080/2]) % [left bottom width height]
            % figure('Position', [0 0 1920/8 1080]) % [left bottom width height] % fig4b, revised fig3d
            figure('Position', [0 1080-1080/3*2-80 1920/8 1080/3*2]) % [left bottom width height] % original fig3d
...
%         end
    ...
    case {'corr_behav_sub1920', 'corr_behav_sub51000', 'corr_behav_sub101000', 'corr_behav_sub151000', 'corr_behav_sub201000'}
        ...
        for bhvi = 1:length(BEHAV)
            ...
            c_mat = squeeze(coef_mat(:, :, bhvi, :)); c_mat_sig = c_mat; % N_iter x N_ROI x N_event
            p_mat = squeeze(pval_mat(:, :, bhvi, :));
            nIter = size(c_mat, 1);
            ...
            if length(EVENT) > 1
                ...
                % c_mat & p_mat: N_iter x N_ROI
                nSig = sum(sig_ind, 1); % 1 x N_ROI
                beeX1 = []; beeX2 = [];
                beeY1 = []; beeY2 = [];
                for ri = 1:N_ROI
                    tempX1 = ri * ones(nSig(ri), 1);
                    tempX2 = ri * ones(nIter - nSig(ri), 1);
                    tempY1 = c_mat(sig_ind(:, ri), ri);
                    tempY2 = c_mat(~sig_ind(:, ri), ri);
                ...
                beeswarm(beeX1, beeY1, ...
                    'colormap', repmat(temp_color, sum(nSig>0), 1), ...
                    'overlay_style', false, ...
                    'corral_style', 'random', ...
                    'MarkerFaceAlpha', .5);
                hold on
                ...
                beeswarm(beeX2, beeY2, ...
                    'colormap', repmat([.5 .5 .5], sum((nIter-nSig)>0), 1), ...
                    'overlay_style', false, ...
                    'corral_style', 'random');
                % all results
                beeswarm(reshape(repmat(1:N_ROI, nIter, 1), [], 1), ...
                    reshape(c_mat, [], 1), ...
                    'colormap', repmat([.5 .5 .5], nIter*N_ROI, 1), ...
                    'overlay_style', 'box2', ...
                    'corral_style', 'gutter', ...
