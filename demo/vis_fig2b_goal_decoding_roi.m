...
% multiple testing correction or not
...
%                     'Alpha', sgnf_lvl);
        ...
        B = bar(msrmat_sbjmean', .6, 'EdgeColor', 'none'); axis square; box off;
        ...
        elseif length(vLABELi) == 10
            ...
            diff_erbr = 0; % ten labels, eight ROIs
            errLineWidth = 2;
        end
...
%                 pvals = nan * ones(size(lab_pairs, 1), 1);
                ...
                for pairi = 1:size(lab_pairs, 1)
                    ...
                    [h, p] = ttest(msrmat_roi(:, lab_pair(1)), ...
                ...
                subplot(2, ceil(length(vROIi)/2), vroii)
                imagesc(lab_pair_sig, [0 2]);
                xticks(1:length(vLABELi))
                xticklabels(LABEL(vLABELi))
                yticks(1:length(vLABELi))
                yticklabels(LABEL(vLABELi))
                text(lab_pairs(:,2)', lab_pairs(:,1)', pvals', ...
                    'HorizontalAlignment', 'center', ...
                    'Color', 'w', ...
                    'FontSize', 9)
                title([ROI{vROIi(vroii)} ' pairwise t-test p-values'])
