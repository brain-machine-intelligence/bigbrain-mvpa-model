...
% ROI = {'F3TL', 'F3TR', 'DLPFCL', 'DLPFCR', 'OFC'};
...
elseif strcmp(measure, 'accs') || strcmp(measure, 'acc_mean')
    ...
    if contains(Exp, 'Lee2014')
        y_lower = chance_lvl - 0.025;
        y_upper = chance_lvl + 0.06; % uncertainty decoding
    elseif contains(Exp, 'Kim2019')
        y_lower = chance_lvl - 0.03;
    ...
    case 'temporal_info'
        ...
        for vroii = 1:length(vROIi)
            ...
            if length(vLABELi) == 2                
                ...
                % For dim2024 uncertaintyDecoding
                msrmat1 = squeeze(msrmat(:,1,vroii,:)); % N_sbj x N_event
                chance1 = chance_lvl(1) * ones(size(msrmat1));
                msrmat2 = squeeze(msrmat(:,2,vroii,:)); % N_sbj x N_event
                chance2 = chance_lvl(2) * ones(size(msrmat2));

                % Original - paired t-test
                sig_event1 = nan * ones(1, length(vEVENTi));
                sig_text1 = cell(1, length(vEVENTi));
                for vevi = 1:length(vEVENTi)
                    [h, p] = ttest(msrmat1(:,vevi)', chance1(:,vevi)', ...
                        'Alpha', sgnf_lvl);
                    if h; sig_event1(vevi) = h; end
                    if p < 0.001
                        sig_text1{vevi} = '***';                         
                    elseif p < 0.01
                        sig_text1{vevi} = '**';                        
                ...
                sig_event2 = nan * ones(1, length(vEVENTi));
                sig_text2 = cell(1, length(vEVENTi));
                for vevi = 1:length(vEVENTi)     
                    [h, p] = ttest(msrmat2(:,vevi)', chance2(:,vevi)', ...
                        'Alpha', sgnf_lvl);
                    if h; sig_event2(vevi) = h; end
                    if p < 0.001
                        sig_text2{vevi} = '***';                         
                    elseif p < 0.01
                        sig_text2{vevi} = '**';                        
                ...
                mean_msr_vec1 = squeeze(msrmat_sbjmean(1, vroii, :)); % N_label x N_event
                ste_msr_vec1 = squeeze(msrmat_sbjste(1, vroii, :)); % N_label x N_event
                text(find(~isnan(sig_event1)), ...
                    mean_msr_vec1(~isnan(sig_event1))+mean(ste_msr_vec1)*1.5, ...
                    sig_text1(~isnan(sig_event1)), ... 
                    'FontSize', 20, ...
                ...
                mean_msr_vec2 = squeeze(msrmat_sbjmean(2, vroii, :)); % N_label x N_event
                ste_msr_vec2 = squeeze(msrmat_sbjste(2, vroii, :)); % N_label x N_event
                text(find(~isnan(sig_event2)), ...
                    mean_msr_vec2(~isnan(sig_event2))+mean(ste_msr_vec2)*1.5, ...
                    sig_text2(~isnan(sig_event2)), ... 
                    'FontSize', 20, ...
                    'HorizontalAlignment', 'center', ...
                    'Color', [0.15 0.1 0.9])
...
%                     plot(msrmat1', 'LineWidth', 0.01, 'Marker', 'o', 'Color', [.9 .9 .9])
            ...
            EB = errorbar(repmat(1:length(vEVENTi), length(vLABELi), 1)', ...
                ...
                'LineWidth', 3); 
            if length(vLABELi) == 2
                EB(1).Color = [0.85 0.15 0.25];
                EB(2).Color = [0.15 0.1 0.9];
            end
            
            % Draw the chance level line
            line([0 length(vEVENTi)], [chance_lvl(1) chance_lvl(1)], ...
                'Color', [.5 .5 .5], ...
                'LineStyle', "--", ...
                'LineWidth', 1.5)
