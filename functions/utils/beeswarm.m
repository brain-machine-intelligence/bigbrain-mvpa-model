...
% extra parameters
% rwid = .15; % width of overlay box/dash
rwid = .1; % width of overlay box/dash
...
if isfinite(p.Results.dot_size)
    if ~p.Results.use_current_axes
        % make new axes
        s=scatter(x,y,.01,'MarkerEdgeColor','none');
        xl=[min(x)-.5 max(x)+.5];
    else
        xl=xlim();
    end
...
if isfinite(p.Results.dot_size)
    ...
    for i=1:length(ux)
        ...
        switch lower(p.Results.overlay_style)
            case 'box2'
                iqr = prctile(yorig(ic==i),[2.5 97.5]);
                rectangle('Position',[ux(i)-rwid iqr(1) 2*rwid iqr(2)-iqr(1)],'EdgeColor','k','LineWidth',2)
                line([ux(i)-rwid ux(i)+rwid],[1 1]*median(yorig(ic==i)),'LineWidth',3,'Color','k')
            ...
            case 'cik'
                line([1 1]*ux(i),mean(yorig(ic==i))+[-1 1]*std(yorig(ic==i))/sqrt(sum(ic==i))*tinv(0.975,sum(ic==i)-1),'Color','k','LineWidth',2)
                line([ux(i)-2*rwid ux(i)+2*rwid],[1 1]*mean(yorig(ic==i)),'LineWidth',3,'Color','k')
            case 'ci'
