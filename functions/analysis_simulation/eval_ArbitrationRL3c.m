function [data_out] = eval_ArbitrationRL3c(param_in, data_in, mode)
...
switch mode.param_length
    ...
    case 8
        ...
        if mode.tau_unc
            tau_uL = param_in(pop_id,5); % inverse temperature in low uncertainty
            tau_uH = param_in(pop_id,7); % inverse temperature in high uncertainty
        end

        if mode.lr_unc
            alpha_uL = param_in(pop_id,6); % learning rate in low uncertainty
            alpha_uH = param_in(pop_id,8); % learning rate in high uncertainty
        end
...
% %% Devaluation/coningency degradation/risk schedule (seems useless 2023-11-01 by ydsung)
...
for ind_dev=[1:1:length(pt_devaluation)] % devaluation loop (seems useless 2023-11-01 by ydsung)
...
%                 disp(sprintf('- generating regressor for SBJ%02d/%02d...',ll,tot_num_sbj));
            ...
            %% Initalization: agent and environment
            ...
            %% (1) phase 1 - 'pre' training (random action, no reward)
            
            if mode.experience_sbj_events(1) == 1
                % using saved behavior data
                num_max_trial0=size(data_in{1,ll}.HIST_behavior_info_pre{1,1},1); % N_trial
                map0.data=data_in{1,ll}.HIST_behavior_info_pre{1,1};  
                map0_s.data=data_in{1,ll}.HIST_behavior_info_pre{1,1};
            else
                % virtual simulation
                num_max_trial0 = 100; % same with Lee2014 subjects (2023-11-01 ydsung)
            end
            ...
            if(mode.experience_sbj_events(1)~=-1)
                ...
                state_sarsa.Qy=state_sarsa.Q;
                state_sarsa.Qw=state_sarsa.Q;
                ...
                while ((i<num_max_trial0)&&(cond))
                    ...
                    if(block_condition==1) % G
                        ...
                        if mode.tau_unc % tau modulation by low uncertainty
                            myArbitrator.tau_softmax = tau_uL;
                            param_sarsa.tau = tau_uL;
                            param_fwd.tau = tau_uL;
                        end
                        if mode.lr_unc % lr modulation by low uncertainty
                            param_sarsa.alpha = alpha_uL;
                            param_fwd.alpha = alpha_uL;
                        end
                    end
                    if(block_condition==2) % G'
                        % set T_prob
                        prob_seed_mat=[0.5 0.5];
                        if mode.tau_unc % tau modulation by high uncertainty
                            myArbitrator.tau_softmax = tau_uH;
                            param_sarsa.tau = tau_uH;
                            param_fwd.tau = tau_uH;
                        end
                        if mode.lr_unc % lr modulation by high uncertainty
                            param_sarsa.alpha = alpha_uH;
                            param_fwd.alpha = alpha_uH;
                        end
                    end
                    ...
                    if(data_in{1,ll}.map_type==1)
                        ...
                        map0.action(1,1).prob(2,[7 8])=prob_seed_mat;
                        map0.action(1,1).prob(3,[8 9])=prob_seed_mat;
                        map0.action(1,1).prob(4,[7 6])=prob_seed_mat;
                        map0.action(1,1).prob(5,[7 9])=prob_seed_mat;
                        map0.action(1,2).prob(1,[4 5])=prob_seed_mat;
                        map0.action(1,2).prob(2,[8 9])=prob_seed_mat;
                        map0.action(1,2).prob(3,[7 9])=prob_seed_mat;
                        map0.action(1,2).prob(4,[6 9])=prob_seed_mat;
                        map0.action(1,2).prob(5,[9 6])=prob_seed_mat;
                    end
                    if(data_in{1,ll}.map_type==2)
                        % T_prob encoding to the current map
                        for mm=1:1:2
...
%                 if(mode.experience_sbj_events(1)==0)                    num_max_trial0=num_max_trial0/2;                end
                while ((i<num_max_trial0)&&(cond))
                    ...
                    if mode.ori == 1
                        ...
                        goal_color = map0.data(map0.trial,18);
                        % Q switch for each colour
                        switch goal_color
                            case 6 % red
                                state_sarsa.Q = state_sarsa.Qr;
                                state_sarsa.goal_color_ind = 1;
                            case 7 % blue
                                state_sarsa.Q = state_sarsa.Qb;
                                state_sarsa.goal_color_ind = 2;
                            case 8 % yellow
                                state_sarsa.Q = state_sarsa.Qy;
                                state_sarsa.goal_color_ind = 3;
                            case -1 % white
                                state_sarsa.Q = state_sarsa.Qw;
                                state_sarsa.goal_color_ind = 4;
                        end
                    ...
                    % configuration adjust (applied only for "LIST_SBJ2" in SIMUL_arbitration_reg
                    % ressor_generator_v1.m                    
                    block_condition=data_in{1,ll}.HIST_block_condition_pre{1,1}(2,i);
                    if(block_condition==1) % G
                        % set T_prob
                        prob_seed_mat=[0.9 0.1];
                    ...
                    if(data_in{1,ll}.map_type==1)
                        ...
                        map0_s.action(1,1).prob(2,[7 8])=prob_seed_mat;
                        map0_s.action(1,1).prob(3,[8 9])=prob_seed_mat;
                        map0_s.action(1,1).prob(4,[7 6])=prob_seed_mat;
                        map0_s.action(1,1).prob(5,[7 9])=prob_seed_mat;
                        map0_s.action(1,2).prob(1,[4 5])=prob_seed_mat;
                        map0_s.action(1,2).prob(2,[8 9])=prob_seed_mat;
                        map0_s.action(1,2).prob(3,[7 9])=prob_seed_mat;
                        map0_s.action(1,2).prob(4,[6 9])=prob_seed_mat;
                        map0_s.action(1,2).prob(5,[9 6])=prob_seed_mat;
                    end
                    if(data_in{1,ll}.map_type==2)
                        % T_prob encoding to the current map
                        for mm=1:1:2
                    ...
                    while (~state_sarsa.JobComplete)
                        ...
                        % 1. sarsa model upate
                        if mode.ori == 2
                            state_sarsa=Model_RL2(state_sarsa, param_sarsa0, map0_s, 'sarsa_3q_update');
                        else
...
%             param_fwd.alpha=param_fwd.alpha/0.5;
            ...
            % else
            %     % virtual simulation
            %     num_max_session = 5; % same with Lee2014 subjects (2023-11-01 ydsung)
            % end
            ...
            for ind_sess=1:1:num_max_session
                myArbitrator_top.HIST_state = []; % (ydsung)
                myArbitrator_top.HIST_Q_chosen = []; % (ydsung)
                myArbitrator_top.HIST_Q_unchosen = []; % (ydsung)
                HIST_NegLogLik = []; % (ydsung)
                myArbitrator_top.HIST_selac = [];
                myArbitrator_top.HIST_mode = [];
                ...
                % if mode.experience_sbj_events(2) == 1
                    ...
                    map.data=data_in{1,ll}.HIST_behavior_info{1,ind_sess}; % (nTrialSession, 18)
                % else
                %     num_max_trial = 80; % same with Lee2014 subjects (2023-11-01 ydsung)
                %     map.data=data_in{1,ll}.HIST_behavior_info{1,ind_sess}; % temporal insertion (2023-11-01 ydsung)
                % end
                ...
                while ((i<num_max_trial)&&(cond))
                    ...
                    if(data_in{1,ll}.map_type==1)
                        % T_prob encoding to the current map
                        map.action(1,1).prob(1,[2 3])=prob_seed_mat;
                        map.action(1,1).prob(2,[7 8])=prob_seed_mat;
                        map.action(1,1).prob(3,[8 9])=prob_seed_mat;
                        map.action(1,1).prob(4,[7 6])=prob_seed_mat;
                        map.action(1,1).prob(5,[7 9])=prob_seed_mat;
                        map.action(1,2).prob(1,[4 5])=prob_seed_mat;
                        map.action(1,2).prob(2,[8 9])=prob_seed_mat;
                        map.action(1,2).prob(3,[7 9])=prob_seed_mat;
                        map.action(1,2).prob(4,[6 9])=prob_seed_mat;
                        map.action(1,2).prob(5,[9 6])=prob_seed_mat;
                    end
...
%                                 vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator_top.Q(1,action_chosen)]; % session; block; trial; trial_s; goal; active_model; choice prob of m1                                
                        ...
                        if mode.experience_sbj_events(2) == 1 % ydsung
                            HIST_NegLogLik = cat(2, HIST_NegLogLik, -eval_num);
                        end
                        ...
                        if(myArbitrator_top.ind_active_model==1) % fwd
                            ...
                            if(mode.experience_sbj_events(2)==1)
                                ...
                                % state_fwd=Model_RL2(state_fwd, param_fwd, map, 'fwd_update', myArbitrator_top, state_sarsa); % (2023-11-01 ydsung)
                            end
                            ...
                            % 3. sarsa model upate
                            if mode.ori == 2
                                state_sarsa=Model_RL2(state_sarsa, param_sarsa, map, 'sarsa_3q_update');
                            elseif mode.ori ==3
                                state_sarsa=Model_RL2(state_sarsa, param_sarsa, map, 'sarsa_3q_update');
                                state_sarsa_idi=Model_RL2(state_sarsa_idi, param_sarsa, map, 'sarsa_update');
                            else
                            ...
                            a_temp = state_fwd.action_history(state_fwd.index - 1); % (ydsung)
                        end
                        ...
                        if(myArbitrator_top.ind_active_model==2) % sarsa
                            ...
                            a_temp = state_sarsa.action_history(state_sarsa.index - 1); % (ydsung)
                        end
                        
                        % Recording before 'myArbitrator' update!
                        s_temp = myArbitrator_top.state_history(state_fwd.index - 1); % (ydsung)
                        % *** (Why -1?) state_fwd/sarsa.index is already
                        % updated (+1) by StateSpace_v1.m

                        % *** (Why no use of myArbitrator_top.action_history?) 
                        % action selection & state transition is already 
                        % done above (by Model_RL2.m and StateSpace_v1.m), 
                        % but Bayesian_Arbitration_v4.m make another action
                        % (which may not be consistent with the state
                        % transition (e.g. (S1,left,S5) )

                        myArbitrator_top.HIST_state = [myArbitrator_top.HIST_state s_temp]; % (2023-11-05 ydsung)
                        myArbitrator_top.HIST_Q_chosen = [myArbitrator_top.HIST_Q_chosen, ...
                                                            myArbitrator_top.Q(s_temp, a_temp)];
                        myArbitrator_top.HIST_Q_unchosen = [myArbitrator_top.HIST_Q_unchosen, ...
                                                            myArbitrator_top.Q(s_temp, mod(a_temp, 2) + 1)];
                        myArbitrator_top.HIST_selac = [myArbitrator_top.HIST_selac a_temp];
                        myArbitrator_top.HIST_mode = [myArbitrator_top.HIST_mode myArbitrator_top.ind_active_model];
...
%                         [myArbitrator_top, state_fwd, state_sarsa]=Bayesian_Arbitration_v3(myArbitrator_top, state_fwd, state_sarsa, map); % full bayesian 
                        ...
                        % HIST_selac2 = cat(2, HIST_selac2, myArbitrator_top);

                        % fprintf('(eval) S,A: %d,%d ', s_temp, a_temp); if map.index == 3; disp(' '); end
...
%                                 vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator_top.Q(state_data,action_chosen)]; % session; block; trial; trial_s; goal; active_model; x                                
                ...
                sbj{ind_sess} = struct('HIST_ArbState', myArbitrator_top.HIST_state, ... 
                                        'HIST_ArbQcho', myArbitrator_top.HIST_Q_chosen, ...
                                        'HIST_ArbQunc', myArbitrator_top.HIST_Q_unchosen, ...
                                        'HIST_ArbAction', myArbitrator_top.HIST_selac, ... 
                                        'HIST_ArbMode', myArbitrator_top.HIST_mode, ...
                                        'HIST_NegLogLik', HIST_NegLogLik);
