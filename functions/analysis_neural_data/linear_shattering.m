...
%     parse(p,pattern,label,varargin{:});
    ...
    MinimalClassSize = options.('MinimalClassSize');
                            ...
                            if isempty(Kfold)
                                switch CVtype
                                    case 'LOROV'
                                        ...
                                        [clabSet, lenCCs] = unq_elms(temp_CClabel);
                                        
                                        % #################################
                                        if options.TrainingSchemeCheck
                                            % New figure for each new label
                                            figure; sbp_cnt = 1; % training scheme check to validate CCGP run
                                        end
                                        % #################################
                                        ...
                                        for run = runSet
                                            ...
                                            accs_r_run_con = nan(rRepeat, length(clabSet));
                                            ...
                                            for cci = 1:length(clabSet)
                                                ...
                                                % #################################
                                                if options.TrainingSchemeCheck
                                                    % [(# of sessions) x (# of cross-condition)]
                                                    subplot(5, 2, sbp_cnt); imagesc([label/4; new_label; temp_CClabel; test_idx; train_idx]); sbp_cnt = sbp_cnt + 1; % training scheme check to validate CCGP run
                                                end
                                                % #################################
...
%                                                     end
                                                ...
                                                if rRepeat
                                                    for rr = 1:rRepeat
                                                        ...
                                                        MdlR = fitclinear(MVP_train, labTrainPerm, ...
                                                            'Learner', 'svm', 'ObservationsIn', 'columns', 'Regularization', 'ridge', 'Solver', 'dual');
                                                        accs_r_run_con(rr, cci) = 1 - loss(MdlR, MVP_test', labTestPerm');
                                                    end
                                                    % accs_r_run_c(:, cci) = accs_r_temp; 
                                                else
                                                    accs_r_run_con = repmat(.5, rRepeat, length(clabSet));
                                                    % accs_r_run_c(:, cci) = repmat(1/N_allclass(labi), rRepeat, 1);
                                                end

                                            end
                                            % row * col / scalar
                                            accs_run(run) = accs_con * lenCCs / sum(lenCCs);
                                            % rows (mat) * col / scalar
                                            if WeightSave; betas_run(:, run) = beta_con * lenCCs / sum(lenCCs); end % (RHS) ( (nVox, nClass) x (nClass, 1) / scalar )
                                            sample_size(r, run) = length(label_train);
                                            
                                            accs_r_run(:, run) = accs_r_run_con * lenCCs / sum(lenCCs); % (accs_r_run: rRepeat, nRun)
                                            % (RHS) ( (rRepeat, nCclass) x (nCclass, 1) / scalar )
                                            % disp(accs_r_run)
                                            % disp(nanmean(accs_r_run(:)))

                                        end
                                        % 'accs' computation, only using valid runs
                                        accs(r) = accs_run(~isnan(accs_run)) * lenRuns(~isnan(accs_run)) ... 
                                            / sum( lenRuns(~isnan(accs_run)) );
                                        accs_per_run(r,:) = accs_run;
                                        if WeightSave
                                            betas(:, r) = betas_run(:, all(~isnan(betas_run), 1)') * lenRuns(all(~isnan(betas_run), 1)') ... 
                                                / sum( lenRuns(all(~isnan(betas_run), 1)') ); % (RHS) ( (nVox, nRun) x (nRun, 1) / scalar )'
                                        end
                                        if WeightSave; betas_per_run(r,:,:) = betas_run; end
                                        accs_rr(r,:) = ( accs_r_run * lenRuns / sum(lenRuns) )'; % (RHS) ( (rRepeat, nRun) x (nRun, 1) / scalar )'
                                end
                                ...
                                if isempty(CClabel)
                                    ...
                                    accs(r) = accs_con * lenCCs / sum(lenCCs);
...
%                             if ~isempty(Exp); CASE.accs_run_mat(com, p_cnt, :) = accs_run_mat; end
                    ...
                    CASE.under_sample_sizes{com, p_cnt} = sample_size;
                end % p_cnt
            end % p
        end % com
