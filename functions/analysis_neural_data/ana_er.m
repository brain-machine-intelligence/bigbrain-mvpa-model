...
for roii = 1:length(ROI)
    ...
    for labi = 1:length(LABEL)
        ...
        for evi = 1:length(EVENT)
            ...
            % We will store run-level effective ranks in a 3D matrix:
            er_run_mat = nan(Repeat, nRuns, nClasses);
            ...
            for rpi = 1:Repeat
                ...
                for runi = 1:nRuns
                    ...
                    runER = nan(1, nClasses);  % eventually fill only the relevant classes
                    ...
                    for cIdx = 1:nRunClasses
                        ...
                        maskC = (idxClass == cIdx);
                        X_c   = MVP_run(:, maskC);  % (nVoxel, nSampleInClass)

                        [~, ~, ~, ~, explained] = pca(X_c'); % (nSample, nFeature)
                        explained = explained / 100;
                        er = exp(explained' * -log(explained));

                        % Identify the global index of this class among allClasses
                        classi = find(allClasses == thisClass);
            ...
            for rpi = 1:Repeat
                for c = 1:nClasses
                    ...
                    v = classRunVal(validMask);      % dispersion values for valid runs (row)
                    er_repeat(rpi, c) = (v * w) / sum(w);
                end
            end

            % --- Final: produce a single measure per class across repeats (optional) ---
            % For example, you can average across repeats:
            ER = mean(er_repeat, 1, 'omitnan')';  % (nClasses x 1) column vector

            % === Save the results in your "save_info" structure, just like you did before ===
            save_info.ER = ER;          % (nClasses x 1) final average
